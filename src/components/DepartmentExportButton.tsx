...
import { exportDepartmentWeeklyReportPDF, exportDepartmentCSV, getWeekDates, getMonthDates } from '../utils/departmentExport';
...
export function DepartmentExportButton({ entries, projects, profiles, departmentName }: DepartmentExportButtonProps) {
  ...
  const handleExport = async () => {
    ...
    try {
      if (reportType === 'week') {
        ...
        if (exportFormat === 'pdf') {
          await exportDepartmentWeeklyReportPDF(weekEntries, projects, profiles, start, end, departmentName);
        } else if (exportFormat === 'csv') {
          exportDepartmentCSV(weekEntries, projects, profiles, start, end, departmentName);
        }
    ...
    <>
      ...
      {showModal && (
        <Modal
          ...
          title={reportType === 'week' ? '匯出部門週報' : '匯出部門月報'}
        >
          <div className="space-y-4">
            {/* 部門資訊 */}
            <div className="p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
              <p className="text-sm text-blue-700 dark:text-blue-400">
                <span className="font-medium">匯出範圍：</span>{departmentName}
              </p>
              <p className="text-xs text-blue-600 dark:text-blue-400 mt-1">
                將匯出 {profiles.length} 位成員的工時記錄
              </p>
            </div>
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">
                {exportFormat === 'pdf' && 'PDF 格式包含員工明細、統計圖表與摘要'}
                {exportFormat === 'csv' && 'CSV 格式適合 Excel 分析，包含所有成員工時明細'}
              </p>
            </div>
