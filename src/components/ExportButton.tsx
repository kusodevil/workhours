...
export function ExportButton({ entries, projects, userName }: ExportButtonProps) {
  ...
  const handleExport = async () => {
    ...
    try {
      if (reportType === 'week') {
        ...
        if (exportFormat === 'pdf') {
          await exportWeeklyReportPDF(weekEntries, projects, start, end, userName);
        } else if (exportFormat === 'csv') {
          exportCSV(weekEntries, projects, start, end);
        }
        ...
        if (exportFormat === 'pdf') {
          await exportMonthlyReportPDF(monthEntries, projects, start, end, userName);
        } else if (exportFormat === 'csv') {
          // 月報的 CSV 使用週報的 CSV 格式（簡單列表）
          exportCSV(monthEntries, projects, start, end);
        }
      }
    ...
    <>
      ...
      {showModal && (
        <Modal
          ...
          title={reportType === 'week' ? '匯出週報' : '匯出月報'}
        >
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">
                {exportFormat === 'pdf' && 'PDF 格式適合正式報表和存檔，支援中文顯示'}
                {exportFormat === 'csv' && 'CSV 格式適合程式處理和匯入到 Excel'}
              </p>
            </div>
